<div class="pb-20 space-y-8">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div
            class="relative flex flex-col justify-between p-6 overflow-hidden bg-white border border-gray-200 shadow-sm dark:bg-gray-900 dark:border-gray-800 rounded-3xl group">
            <div
                class="absolute transition-transform -right-6 -top-6 text-indigo-50 dark:text-indigo-900/20 group-hover:scale-110">
            ...
            <div class="relative z-10">
                ...
                <h3 class="mb-1 text-2xl font-black text-gray-900 dark:text-white">Rp
                    {{ number_format($totalPendapatan, 0, ',', '.') }}</h3>
                <p
                    class="text-[10px] text-emerald-500 font-bold bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded-md inline-block">
                    Omset Penjualan Kotor</p>
            </div>
        </div>
        ...
        <div
            class="relative flex flex-col justify-between p-6 overflow-hidden bg-white border border-gray-200 shadow-sm dark:bg-gray-900 dark:border-gray-800 rounded-3xl group">
            <div
                class="absolute transition-transform -right-6 -top-6 text-red-50 dark:text-red-900/20 group-hover:scale-110">
            ...
            <div class="relative z-10">
                ...
                <p
                    class="text-[10px] text-red-500 font-bold bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded-md inline-block">
                    Total Modal Barang (FEFO)</p>
            </div>
        </div>

        <div
            class="relative flex flex-col justify-between p-6 overflow-hidden text-white bg-indigo-600 border border-indigo-700 shadow-md rounded-3xl group">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[10px] text-white font-bold bg-indigo-500/50 px-2 py-1 rounded-md inline-block">
                    Pendapatan - HPP</p>
            </div>
        </div>
        ...
        <div
            class="relative flex flex-col justify-between p-6 overflow-hidden bg-white border border-gray-200 shadow-sm dark:bg-gray-900 dark:border-gray-800 rounded-3xl group">
            <div
                class="absolute transition-transform -right-4 -top-4 text-emerald-50 dark:text-emerald-900/20 group-hover:scale-110">
            ...
            <div class="relative z-10">
                ...
                <h3
                    class="text-3xl font-black {{ $marginPersen >= 40 ? 'text-emerald-500' : ($marginPersen >= 20 ? 'text-yellow-500' : 'text-red-500') }} mb-1">
                    {{ $marginPersen }}%
                </h3>
    ...
    <div
        class="overflow-hidden bg-white border border-gray-200 shadow-sm dark:bg-gray-900 rounded-3xl dark:border-gray-800">
        <div
            class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
            ...
            <div class="flex items-center gap-2">
                ...
                <select wire:model.live="view"
                    class="px-3 py-1.5 text-xs font-bold border-none bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white">
        ...
        <div class="relative overflow-x-auto">
            <div wire:loading.flex
                class="absolute inset-0 z-10 items-center justify-center transition-all bg-white/60 dark:bg-gray-900/60 backdrop-blur-[2px]">
            ...
            <table class="w-full font-mono text-sm text-left border-collapse">
                <thead>
                    <tr
                        class="text-xs text-gray-400 uppercase border-b border-gray-100 bg-gray-50 dark:bg-gray-800/50 dark:border-gray-800">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/40">
                        ...
                        <td class="px-6 py-4 font-bold text-center text-indigo-600 dark:text-indigo-400">
                            {{ $item->qty_terjual }}</td>
                        <td class="px-6 py-4 text-right">Rp {{ number_format($item->pendapatan_produk, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 text-right text-gray-500">Rp
                            {{ number_format($item->hpp_produk, 0, ',', '.') }}</td>
                        <td class="px-6 py-4 font-bold text-right text-emerald-600 dark:text-emerald-400">Rp
                            {{ number_format($laba, 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-center">
                            <span
                                class="px-2 py-1 rounded text-[10px] font-bold {{ $margin >= 40 ? 'bg-emerald-100 text-emerald-700' : ($margin >= 20 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
                                {{ $margin }}%
                            </span>
                        </td>
                    </tr>
