        ...
        " style="transform: translateY(calc(100% - 70px));">
            ...
            <div class="absolute lg:relative bottom-0 left-0 right-0 bg-white dark:bg-gray-800 p-4 border-t border-gray-100 dark:border-gray-700 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.1)] lg:shadow-none lg:rounded-b-2xl transition-colors">
                <div class="flex items-center gap-4">
                    ...
                    <button onclick="checkout()" class="flex items-center justify-center flex-1 gap-2 px-4 py-3 font-bold text-white transition-all shadow-lg bg-brand-red hover:bg-red-700 rounded-xl active:scale-95">
                        <span>Pesan</span>
...
<script type="text/javascript">
    ...
    // --- STATE ---
    window.appState = window.appState || { bowls: [], activeBowlId: null, activeCategory: 'all', searchQuery: '' };
    window.isCartOpen = window.isCartOpen || false;
    // --- INIT ---
    function homeInit() {
        window.savedData = localStorage.getItem('pos-enterprise');
    ...
    function renderCategories() {
        document.getElementById('category-container').innerHTML = CATEGORIES.map(cat => {
            ...
            const classes = isActive ? 'bg-brand-red text-white ' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-700';
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-1 gap-3 p-3 border border-gray-100 rounded-lg bg-gray-50 dark:bg-gray-900 dark:border-gray-700">
                            <div>
                                ...
                                <select onchange="updatePreference('${bowl.id}', 'kuah', this.value)" class="w-full p-2 mt-1 text-xs text-gray-800 bg-white border border-gray-200 rounded rounded-lg outline-none cursor-pointer dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 focus:border-brand-red">
                            ...
                            <div>
                                <label class="text-[10px] uppercase font-bold text-gray-400 tracking-wider flex items-center gap-1"><i class="text-xs fa-solid fa-pencil"></i> Catatan Khusus</label>
                                <textarea onchange="updateBowlNote('${bowl.id}', this.value)" class="w-full p-2 mt-1 text-xs text-gray-800 bg-white border border-gray-200 rounded outline-none resize-none dark:bg-gray-800 dark:border-gray-600 dark:text-white focus:border-brand-red" rows="2" placeholder="Contoh: Jangan pake bawang...">${bowl.note || ''}</textarea>
                            </div>
                        </div>
